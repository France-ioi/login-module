...
class SchemaBuilder {
        ...
        foreach($visible_attributes as $attribute) {
            ...
            if(isset($config['prepend'])) {
                foreach($config['prepend'] as $attr_ex) {
                    ...
                    $blocks[] = $this->createBlock($attr_ex, $config_ex, $required, $disabled);
                    $added[$attr_ex] = true;
                }
            }
            if(!isset($added[$attribute])) {
                $blocks[] = $this->createBlock($attribute, $config, $required, $disabled);
    ...
    private function createBlock($attribute, $config, $required, $disabled) {
        return (object) [
